...
$ready = $false
try { $health = Invoke-RestMethod -Uri 'http://127.0.0.1:3001/api/health' -TimeoutSec 2; $ready = $health.ok -eq $true -and $health.service -eq 'our-macao' } catch {}
if (-not $ready) {
    ...
    for ($attempt = 0; $attempt -lt 20; $attempt++) {
        ...
        try { $health = Invoke-RestMethod -Uri 'http://127.0.0.1:3001/api/health' -TimeoutSec 2; if ($health.ok -and $health.service -eq 'our-macao') { $ready = $true; break } } catch {}
    }
}
if (-not $ready) { throw 'Local server did not start. Read storage/server-error.log.' }
Write-Host 'Macao: http://127.0.0.1:3001'
